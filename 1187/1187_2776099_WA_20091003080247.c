...
int main(){
    ...
    while(1){
      ...
      for(i=0;i<ql[ta];i++){
        ...
        for(j=0;j<ql[tb];j++){
          if(tot1[i]==0)printf("     -");
          else printf("%5.0lf%c",ge()+(double)count[i][j]*100/tot1[i],'%');
        }
        if(tot1[i]==0)puts("     -");
        else puts("  100%");
        for(j=0;j<6;j++)putchar(' ');
        for(j=0;j<ql[tb];j++){
          if(tot2[j]==0)printf("     -");
          else printf("%5.0lf%c",ge()+(double)count[i][j]*100/tot2[j],'%');
        }
        printf("%5.0lf%c\n",ge()+(double)tot1[i]*100/rs,'%');
      }
      printf(" TOTAL");
      for(j=0;j<ql[tb];j++)printf("%6d",tot2[j]);
      printf("%6d\n",rs);
      for(j=0;j<6;j++)putchar(' ');
      for(j=0;j<ql[tb];j++)printf("%5.0lf%c",ge()+(double)tot2[j]*100/rs);
      puts("  100%");
      for(j=0;j<6;j++)putchar(' ');
      for(j=0;j<ql[tb];j++){
        if(tot2[j]==0)printf("     -");
