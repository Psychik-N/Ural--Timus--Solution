...
#include <stdlib.h> 
#include <time.h>
int n,m,a[150],b[150],pb[150],c[150]={0},d[150]={0},pc[150],v=0,g[150],kb[150],r=999999937;
...
int go2(int x,int left,int s){
    ...
    for(i=s;i<=n;i++){
        ...
        c[i]=pb[x];
        pc[i]=a[i];
        if(left==a[i])j=go2(x+1,b[x+1],g[x+1]);
...
int go3(int x,int left,int s){
    ...
    for(i=s;i<=n;i++){
        ...
        c[i]=pb[x];
        pc[i]=a[i];
        if(left==a[i])j=go3(x+1,b[x+1],g[x+1]);
        else j=go3(x,left-a[i],i+1);
        if(j)return 1;
        c[i]=0;
    }
    return 0;
