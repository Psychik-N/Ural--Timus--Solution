...
#include <stdlib.h> 
#include <time.h>
int n,m,a[150],b[150],pb[150],c[150],d[150],pc[150],g[150],x,left1,s,m1,sf[15],v=0;
int ta[150],tb[150],sp=0,fm[11][150]={0},fl[11][150]={0},bf[11][150]={0};
int mynum[11]={0};
int bag[110][30000]={0},syg[110][30000]={0},from[110][30000]={0},y[110][30000]={0},lbl[160],lbr[160];
int bagskip[110]={0},nomove[15]={0},hold[150]={0},fp[150];
char cr[20000]={1};
void nxt();
...
void go(){
    ...
    while(i<=n){
        ...
        if(c[i]||left1<a[i]||fm[x][a[i]]==mynum[x]){i=lbr[i];continue;}
        f2 = fl[x][0];
        c[i]=pb[x];
        pc[i]=a[i];
        back=s;
...
int main(){
    ...
    for(i=1;i<=n;i++)for(j=1;j<=n;j++)if(a[i+1]>a[j+1]){swap(&a[i],&a[j]);}
    for(i=1;i<=m;i++){
        for(j=1;j<=n;j++){
            if(a[j]<=b[i]){
                sf[i]=j;
