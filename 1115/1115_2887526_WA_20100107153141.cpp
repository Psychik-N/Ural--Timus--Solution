#include<iostream>
using namespace std;
int n,m,a[150],b[150],pa[150],pb[150],c[150]={0},d[150]={0},pc[150];
...
int main(){
    cin>>n>>m;
    int i,j;
    for(i=1;i<=n;i++){cin>>a[i];pa[i]=i;}
    for(i=1;i<=m;i++){cin>>b[i];pb[i]=i;}
    for(i=1;i<=n;i++)for(j=1;j<=n;j++)if(a[i]>a[j]){swap(a[i],a[j]);swap(pa[i],pa[j]);}
