...
struct Edge{
    ...
    int len;
    Edge *next;
    Edge(int to,int type,int len,Edge *next){
        this->to=to;
        this->type=type;
        this->len=len;
        this->next=next;
...
int main(){
    ...
    char st[15];
    scanf("%d%d",&n,&m);
    for(int i=1;i<=n;i++)for(int j=0;j<2;j++)f[i][j]=LLONG_MAX/3;
    f[1][0]=0;
    for(int i=0;i<m;i++){
        ...
        if(st[0]=='L')edge[x][0]=new Edge(y,0,d,edge[x][0]);
            ...
            edge[x][1]=new Edge(y,1,d,edge[x][1]);
        }else{
    ...
    min(f[n][0],f[n][1]);
    if(f[n][0]==LLONG_MAX/3)puts("Offline");
    else printf("Online\n%lld\n",f[n][0]);
    return 0;
}
