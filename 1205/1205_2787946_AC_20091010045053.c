...
for(i=1;i<=n;i++)for(j=1;j<=n;j++){
if(i==j)continue;
if(sqrt((xp-x[i])*(xp-x[i])+(yp-y[i])*(yp-y[i]))/s1+dis[i][j]+sqrt((x2-x[j])*(x2-x[j])+(y2-y[j])*(y2-y[j]))/s1+eps<bv){
bv=sqrt((xp-x[i])*(xp-x[i])+(yp-y[i])*(yp-y[i]))/s1+dis[i][j]+sqrt((x2-x[j])*(x2-x[j])+(y2-y[j])*(y2-y[j]))/s1;
bs=i;
be=j;
}
}
